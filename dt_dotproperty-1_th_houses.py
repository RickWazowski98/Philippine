...
def resolve_graph_link(url_project):
    ...
    if '/house/' in url_project:
        ...
        url = 'https://www.dotproperty.co.th/en/market-stats/project-page/condo/?key={}'.format(id)
        return url
    print('WARNING: cant resolve graph_link for market stats')
...
if __name__ == "__main__":
    file_object = open('data/links_th_prov_houses.json', 'r')
    ...
    for link in links:
        ...
        if result_bulk_1:
            ...
            file_object = open('data/projects_data_th_houses_result_bulk_1.json', 'w')
            json.dump(result_1, file_object, indent=4)
        
        if result_bulk_2:
            result_2.append(result_bulk_2)

            file_object = open('data/projects_data_th_houses_result_bulk_2.json', 'w')
            json.dump(result_2, file_object, indent=4)
        
        if result_bulk_3:
            result_3.append(result_bulk_3)

            file_object = open('data/projects_data_th_houses_result_bulk_3.json', 'w')
